...
# TELOMERES_BY_COHORTS

if [ ! -d "TELOMERES_BY_COHORTS" ]
then
echo "Directory TELOMERES_BY_COHORTS does not exist and will be created"
mkdir -m a=rwx "TELOMERES_BY_COHORTS"
if [ $? -ne 0 ]
then
echo "Can not create folder TELOMERES_BY_COHORTS check permission"
else
echo "Created TELOMERES_BY_COHORTS"
fi
fi
